...
class ArgsParser(ArgumentParser):
    def __init__(self):
        ...
        self.add_argument("-c", "--config", help="configuration file to use")    # 输入模型训练的配置文件
        self.add_argument(
            "-o", "--opt", nargs='+', help="set configuration options")    # 设置训练的选项参数
        self.add_argument(
            '-p',
            '--profiler_options',
            type=str,
...
def preprocess(is_train=False):
    """参数判断是否为训练过程"""
    FLAGS = ArgsParser().parse_args()    # 取出配置文件中的内容
    ...
    if is_train:
        # save_config
        save_model_dir = config['Global']['save_model_dir']    # 如果为训练过程，将模型保存到配置文件指定的路径中
        os.makedirs(save_model_dir, exist_ok=True)    # 创建模型保存的文件路径
        with open(os.path.join(save_model_dir, 'config.yml'), 'w') as f:
            yaml.dump(
                dict(config), f, default_flow_style=False, sort_keys=False)
        log_file = '{}/train.log'.format(save_model_dir)    # 将训练的日志文件保存到train.log中
    else:
        log_file = None
    ...
    alg = config['Architecture']['algorithm']    # 在配置文件中选择网络结构
    assert alg in [
